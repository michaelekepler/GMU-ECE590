...
if len(sys.argv) > 1:
	if sys.argv[1].lower() == 'orange': make_mask = cam.create_mask_orange
...
pid_x = pid.PidController(0, 0.15, 0.01, 0.1, 1.0, time.time(), 6)
count = 0
try:
	while True:
		...
		masked_image = make_mask(image)
		kernel = numpy.ones((1,1),numpy.uint8)
		mask = cv2.dilate(masked_image,kernel,iterations=10)
		...
		if onscreen:
			...
			cmd = 'j200' # pivot left at speed 400 to search
		usb.write(cmd)
		print("\r\x1b[4A", end='')
